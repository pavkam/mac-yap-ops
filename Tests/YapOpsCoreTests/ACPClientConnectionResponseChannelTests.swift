...
extension ACPClientConnectionTests {
        ...
        async throws
    {
        ...
        let task = Task {
            try await ACPClientConnection.connect(
                ...
                diagnostics: YapOpsDiagnostics.shared).connection
        }
        ...
        #expect(metadata["ciobanu.org.yapOps"] != nil)
        ...
        async throws
    {
        ...
        #expect(text(in: blocks[1])?.contains("yapops.agent-continuity.v1") == true)
        #expect(text(in: blocks[2])?.contains("yapops.mac-context.v1") == true)
