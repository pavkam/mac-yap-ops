...
| Deleting a retained conversation shrinks the menu but leaves old rounded strips above and below it | The window-style `MenuBarExtra` resized without recomputing its AppKit shadow | Track structural menu layout identity and call `NSWindow.invalidateShadow()` after the settled resize; guard the host-window side effect with `MenuContentViewTests`. |
