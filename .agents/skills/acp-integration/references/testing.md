...
| Stop cancels but a new reply starts immediately | Pending follow-ups or partial recognition survived cancellation | Retire recognition and execution, discard pending input before publishing cancellation, and assert no next prompt or microphone session starts until explicit resume. Guard with coordinator cancellation and pause tests. |
| New conversations contain old replies | A new run implicitly reused a per-profile cache or saved bookmark | First input requests `.fresh`; bypass both warm cache and durable restoration, while follow-ups retain context. Guard with fresh-conversation runner and coordinator tests, preserving active background tasks. |
